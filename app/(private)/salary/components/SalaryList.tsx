    ...
    <div className="p-6">
      <Card className="bg-white border border-gray-300 shadow-sm">
        ...
        <div className="flex justify-between items-center p-4 bg-gray-100 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-black">Quản Lý Mức Lương</h2>
          <Button
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-b border-gray-200 bg-gray-100">
                ...
                <TableHead className="text-black">Toàn Cầu</TableHead>
                <TableHead className="text-black text-center">Hành Động</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {salaries.map(salary => (
                <TableRow key={salary.id} className="border-b border-gray-200 hover:bg-gray-100">
                  ...
                  <TableCell className="text-gray-700">{salary.isGlobal ? 'Có' : 'Không'}</TableCell>
      ...
      {deleteId && <DeleteSalaryConfirm id={deleteId} onClose={() => setDeleteId(null)} />}
      {isAdding && <AddSalaryModal isOpen={isAdding} onClose={() => setIsAdding(false)} />} {/* ✅ Use AddSalaryModal */}
    </div>
  );
}
